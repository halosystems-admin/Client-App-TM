...
async function loadActiveStylePrompt(poolRef: pg.Pool, templateDbId: string): Promise<StylePromptRow> {
  const activeResult = await poolRef.query<StylePromptRow>(
    `
  ...
  if (activeResult.rows.length > 0) {
    const prompt = activeResult.rows[0].system_prompt_md?.trim();
    if (!prompt) {
      throw new Error('Active style prompt is empty for template.');
    }
    return { ...activeResult.rows[0], system_prompt_md: prompt };
  }

  const fallbackResult = await poolRef.query<StylePromptRow>(
    `
      SELECT system_prompt_md, version
      FROM scribe_style_prompts
  ...
  if (fallbackResult.rows.length === 0) {
    throw new Error('No style prompt configured for template.');
  }
  const prompt = fallbackResult.rows[0].system_prompt_md?.trim();
  if (!prompt) {
    throw new Error('Latest style prompt is empty for template.');
  }

  return { ...fallbackResult.rows[0], system_prompt_md: prompt };
}
...
async function loadSystemFieldRows(poolRef: pg.Pool, templateDbId: string): Promise<SystemFieldRow[]> {
  const result = await poolRef.query<SystemFieldRow>(
    `
      SELECT key, source, default_value
