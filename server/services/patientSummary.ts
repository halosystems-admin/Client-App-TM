...
const SUMMARY_USER_ID = 'patient-summary';
const SUMMARY_SERVICE_NAME = 'Gemini-Patient-Summary';

function patientSummarySourcePrompt(input: {
  patientName: string;
  ...
  return `You are creating one timeline entry for a patient record summary.\n\nPatient: ${input.patientName}\nSource type: ${input.sourceType}\nSource name: ${input.sourceName}\nSource date: ${input.sourceDate}\n\nContent:\n${input.content}\n\nReturn ONLY JSON with shape {\"title\": string, \"bullets\": string[] } and at most 3 bullets.`;
}

function patientSummaryMergePrompt(input: {
  patientName: string;
  currentSnapshot: string[];
  recentTimeline: Array<{ date: string; title: string; bullets: string[] }>;
  newUpdate: { title: string; date: string; bullets: string[] };
}): string {
  return `Update the patient snapshot using the latest timeline data.\n\nPatient: ${input.patientName}\nCurrent snapshot: ${JSON.stringify(input.currentSnapshot)}\nRecent timeline: ${JSON.stringify(input.recentTimeline)}\nNew update: ${JSON.stringify(input.newUpdate)}\n\nReturn ONLY JSON: {\"snapshot\": string[]} with at most 5 concise bullets.`;
}
...
async function extractSummaryEntry(
  ...
  const raw = await generateText(prompt, SUMMARY_USER_ID, SUMMARY_SERVICE_NAME, false);
