...
2. Check health: `https://app.halo.africa/api/health`.
3. Confirm status semantics:
  - `status: "ok"` (HTTP 200): all configured checks are healthy.
  - `status: "partial"` (HTTP 207): app is running but one or more optional integrations are unconfigured.
4. Inspect `checks` in the response payload (example keys: `server`, `gemini`, `deepgram`, `haloApi`, `smtp`).
5. If `smtp` is `unconfigured`, set `SMTP_HOST`, `SMTP_USER`, and `SMTP_PASS` (and optionally `SMTP_PORT`, `SMTP_SECURE`, `ADMIN_EMAIL`) and restart the app.
6. Log in with Google — you should be redirected to Google and back to the app.
7. If something fails, check server logs (e.g. `pm2 logs halo-app`) and the browser devtools (Console / Network).
...
- [ ] `https://<your-host>/api/health` returns expected status (`ok` or `partial`) and login works from desktop and phone
...
- **Health shows `partial`**: Check `checks` in `/api/health` response. For SMTP readiness, set `SMTP_HOST`, `SMTP_USER`, and `SMTP_PASS`. For optional services you do not use, `partial` can be acceptable.
