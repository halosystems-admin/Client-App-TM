...
export const AdmissionsColumn: React.FC<Props> = ({
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className={`flex h-full w-[86vw] max-w-[320px] shrink-0 snap-start flex-col sm:w-[300px] ${isDragging ? 'opacity-75' : ''}`}
    >
          ...
          <div className="space-y-2.5">
            {column.cards.map((card) => (
              <AdmissionsPatientCard
                ...
                onMoveClick={() => onMoveCard?.(card.id)}
                onTriageColorClick={() => onTriageColorClick?.(card.id)}
              />
            ))}
